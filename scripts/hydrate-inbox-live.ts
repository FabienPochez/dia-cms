...
const LOCKFILE_PATH = '/tmp/lt-hydrate-inbox-live.lock'

// Environment variables
const LIBRETIME_CONTAINER_NAME = process.env.LIBRETIME_CONTAINER_NAME || 'libretime-web'
const MEDIA_NEW_LIVE_DIR = process.env.MEDIA_NEW_LIVE_DIR || '/srv/media/new-live'
...
function buildPayloadAuthHeaders(): { Authorization: string; 'Content-Type': 'application/json' } {
  ...
  if (PAYLOAD_API_KEY) {
    console.log('⚠️  Using PAYLOAD_API_KEY for authentication. Consider using PAYLOAD_INBOX_API_KEY for live hydration.')
...
/**
 * Filter episodes by eligibility criteria for LIVE recordings
 * Accepts episodes with airStatus: 'scheduled' or 'aired' (already aired or scheduled to air)
 */
function filterEligibleEpisodes(episodes: PayloadEpisode[]): PayloadEpisode[] {
  const eligible = episodes.filter((episode) => {
    ...
    // airStatus must be 'scheduled' or 'aired' (for live recordings)
    // These are episodes that have already aired or are scheduled to air
    if (episode.airStatus !== 'scheduled' && episode.airStatus !== 'aired') {
      return false
    }
...
/**
 * Update Payload episode with LibreTime track ID and filepath (DOES NOT CHANGE airStatus)
 ...
 */
async function updateEpisodeWithoutAirStatusChange(
  episodeId: string,
  ...
  // Fetch current episode to check if duration fields are already set and to preserve airStatus
  ...
  // Build update payload - DO NOT change airStatus (preserve existing status)
  const updateData: any = {
    libretimeTrackId: libretimeData.id.toString(),
    libretimeFilepathRelative: libretimeData.relativePath,
    // NOTE: We intentionally do NOT set airStatus here - preserve existing status
  }
  ...
  try {
    // Update the episode with LT fields and duration metadata (airStatus unchanged)
    ...
    console.log(`   airStatus: preserved (unchanged)`)
    if (audioMetadata) {
      ...
      try {
        ...
        if (matches.length > 0) {
          ...
          if (libretimeData.relativePath) {
            ...
            // Update Payload if not dry-run (preserves airStatus)
            if (!dryRun) {
              await updateEpisodeWithoutAirStatusChange(episodeId, libretimeData, inboxPath)
            } else {
              console.log(`🔍 DRY-RUN: Would update episode ${episodeId} (airStatus preserved)`)
            }
          } else {
 ...
 * Main function
 */
async function hydrateInboxLive(): Promise<void> {
  console.log('🎧 LibreTime Live Recordings Hydration Script')
  console.log('==============================================')

  const options = parseArgs()

  const inboxPath = options.inbox || MEDIA_NEW_LIVE_DIR
  ...
  try {
    ...
    if (stats.hydrated > 0) {
      console.log('\n🎉 Live recordings hydration completed successfully!')
    } else if (stats.errors > 0) {
      console.log('\n⚠️  Some episodes failed to hydrate (check logs above)')
    }
  } catch (error: any) {
    console.error('\n❌ Live recordings hydration failed:', error.message)
    throw error
  }
}

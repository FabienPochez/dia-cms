...
const LOCKFILE_PATH = '/tmp/lt-bulk-import.lock'
...
/**
 * Auto-detect episode ID from media/new directory
 */
async function autoDetectEpisodeId(): Promise<string> {
  const mediaNewDir = '/srv/media/new'

  try {
    const files = await fs.readdir(mediaNewDir)
    const mp3Files = files.filter((file) => file.endsWith('.mp3'))

    if (mp3Files.length === 0) {
      throw new Error(`No MP3 files found in ${mediaNewDir}`)
    }

    if (mp3Files.length > 1) {
      console.log(`⚠️  Warning: Multiple MP3 files found in media/new:`)
    ...
    throw new Error(`Failed to auto-detect episode ID from media/new directory: ${error.message}`)
  }
}
...
async function resolveAudioFile(episodeId: string, filePath?: string): Promise<string> {
  ...
  // Check media/staging directory first
  const mediaNewDir = '/srv/media/new'
  try {
    const mediaNewFiles = await fs.readdir(mediaNewDir)
    const mediaNewMp3 = mediaNewFiles.find((file) => file.endsWith('.mp3'))
    if (mediaNewMp3) {
      const mediaNewPath = path.join(mediaNewDir, mediaNewMp3)
      console.log(`📁 Found media/new file: ${mediaNewPath}`)
      return mediaNewPath
    }
  } catch {
    // Media/new directory doesn't exist or is empty, continue with normal search
  }
...
async function performSingleBulkImport(directoryPath: string): Promise<void> {
  ...
  const command = `docker exec libretime_api_1 libretime-api bulk_import --path "${directoryPath}" --allowed-extensions "mp3"`
...
async function bulkImportToLibreTime(filePath: string): Promise<void> {
  ...
  try {
    // Option A: Skip metadata stripping, go straight to bulk import
    console.log(`🚀 SKIP_STRIP: Skipping metadata stripping, proceeding to bulk import`)
...
async function importOneEpisode(): Promise<void> {
  console.log('🎧 LibreTime Batch Episode Import Script')
  console.log('==========================================')
  ...
  try {
    ...
    // Step 2: Get directory path (use staging directory)
    const directoryPath = '/srv/media/new'
    ...
    if (!shouldImport) {
      ...
      // Step 4: Import to LibreTime (HTTP by default, CLI as fallback)
      if (options.ingestMode === 'http') {
        if (options.dryRun) {
          console.log('🔍 DRY-RUN: Would upload files to LibreTime via HTTP')
          return
        }
        // For HTTP mode, we'd need to upload each file individually
        console.log('⚠️  HTTP mode not implemented for bulk import, using CLI mode')
      }

      if (options.dryRun) {
        console.log('🔍 DRY-RUN: Would strip metadata and perform bulk import')
        return
      }
